...
type
  TfMain = class(TForm)
    ...
    procedure btnProductsIniSaveClick(Sender: TObject);
...
var i : integer;
    ...
    Factor : double;
begin
  if (ACol<> 7) OR (ARow <> 0) then exit;

  for I := 1 to asgPoly.RowCount -1 do begin
    nValue := strTofloatDef(StringReplace(asgPoly.Cells[6,i],',','',[rfReplaceAll]),0);
    nValue := nValue / 1100;// 환율 1100원
    nValue := Ceil(nValue);
//    DecimalPlaces := 1; // Round up to 1 decimal place
//    Factor := Power(10, DecimalPlaces);
//    nValue := Ceil(nValue * Factor) / Factor;
...
begin
  ...
  try
    ...
    strPoly := '';
    for i := 0 to strTointDef(edtTerms.Text,0) -1 do begin
      strPoly := strPoly + 'C' + i.ToString + ':' + floatTostr(ini.ReadFloat('coef',i.ToString,0.0)) + '  ';
      coef[i] := ini.ReadFloat('coef',i.ToString,0.0);
    end;
    edtPoly.Text := strPoly;
...
begin
  ...
  btnProductsIniSaveClick(Sender);
end;
...
procedure TfMain.btnProductsIniSaveClick(Sender: TObject);
var strTemp : string;
begin
  strTemp := ExtractFilePath(paramstr(0));
...
begin
  ...
  try
    ...
    for I := 0 to strTointDef(edtTerms.Text,0) -1 do begin
      ini.WriteFloat('coef',i.ToString,coef[i]);
    end;
